...
function darkMode() {
    //get dark mode status from localStorage
...
function setActiveSidebarLink() {
    const routeName = JSON.parse(
        document.getElementById("route").dataset.route
    );
    ...
    for (const [parent, children] of Object.entries(routes)) {
        ...
        if (children.length < 1) {
            if (routeName.startsWith(parent))
