...
<body class="bg-slate-50 min-h-screen sans-pro flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            ...
            <form action="{{ route('login') }}" method="POST">
                ...
                <div class="flex items-center justify-between mt-4">
                    ...
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm text-slate-900 hover:text-slate-600 transition">
                            Forgot password?
                        </a>
                    @endif
                </div>
            ...
            <a href="{{ route('teacher.login') }}" class="block mt-3">
                <button
                    class="w-full border border-slate-300 text-slate-900 rounded-lg h-11 font-medium hover:bg-slate-50 transition"
                    type="button">
                    Go to Teacher's login
                </button>
            </a>
