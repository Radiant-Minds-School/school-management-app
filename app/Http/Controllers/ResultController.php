...
class ResultController extends Controller
{
    ...
    public function create(Student $student)
    {
        ...
        $classroomSubjects = $student->classroom
            ->subjects()
        ...
        return view(
            "result.create",
            compact("subjects", "student", "activePeriod"),
        );
    }
    ...
    public function showPerformanceReport(Student $student, string $periodSlug)
    {
        ...
        try {
            ...
            if ($e->getMessage() == "Student's class does not have subjects") {
                return redirect()
                    ->route("classroom.show", [
                        "classroom" => $student->classroom,
                    ])
                    ->with(
                        "error",
                        'The student\'s class does not have subjects set for the selected academic session',
                    );
    ...
    public function update(Result $result, Request $request)
    {
        ...
        return redirect($request->session()->get("resultsPage"))->with(
            "success",
            "Result Updated!",
        );
    }
    ...
    public function mailStudentPerformanceReport(Student $student, $periodSlug)
    {
        if ($student->guardian->email == null) {
            return back()->with(
                "error",
                "Guardian does not have an email address.",
            );
        }

        Mail::to($student->guardian->email)->send(
            new StudentPerformanceReport($student, $periodSlug),
        );
